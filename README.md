...
- `streaming_base/` : contains the base code for streaming and processing the radar data
    ...
    - `streaming/` : contains the code for streaming the radar data and running the real-time pipeline
        ...
        - NB: these two files involve most of the added code for this project
...
### Branches
- `main` : the main branch, with the final version used for the project presentation, demos and all references to code in the final report. *NB: this branch is the most up to date and has the latest code, we cannot garuntee all code to be working in other branches*
...
- `Kasper` : branch used by a team member for testing doppler-related methods on the pipeline.
...
### Troubleshooting:
- ensure the DCA board is connected properly and all connections are solid
    - verify correct ports can be seen by computer
- ensure firewalls are disabled
- ensure no background DCA processes are running
